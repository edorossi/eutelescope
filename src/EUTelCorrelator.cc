        ...
        int col = _sensorIDVec.at( c );
         ...
         if ( 
          ...
          if ( _hasHitCollection ) {
            ...
            colNBin  =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xBin : 200   ;    
            colMin   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xMin : -1.0* geo::gGeometry().siPlaneXSize(row);
            colMax   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xMax :  1.0* geo::gGeometry().siPlaneXSize(row);
            rowNBin  =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yBin : 200   ;    
            rowMin   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yMin : -1.0* geo::gGeometry().siPlaneXSize(col);
            rowMax   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yMax :  1.0* geo::gGeometry().siPlaneXSize(col);
            ...
            colNBin  =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xBin : 200   ;    
            colMin   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xMin : -1.0* geo::gGeometry().siPlaneYSize(row);
            colMax   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_xMax :  1.0* geo::gGeometry().siPlaneYSize(row);
            rowNBin  =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yBin : 200   ;    
            rowMin   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yMin : -1.0* geo::gGeometry().siPlaneYSize(col);
            rowMax   =      ( isHistoManagerAvailable && histoInfo ) ? histoInfo->_yMax :  1.0* geo::gGeometry().siPlaneYSize(col);
